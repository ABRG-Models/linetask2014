...
Johnathan Spencer\textsuperscript{1,\Yinyang},
...
% FIXME: Also, add something about the delays in the display and
% delay/jitter in the readout of the stylus position.
%
...
greater than 0.05 pixels per millisecond (50~px/s; 0.57~cm/s,
...
% We should probably understand this 3 ms difference better...
...
%The null
%hypothesis of the Shapiro-Wilks test is that the data are normally
%distributed.
...
applied. %to determine if the variation in the latencies was due to the
%condition, individual effects or random effects.
%We applied an
%analysis on the latency ranks, to avoid the need for the normality
%assumption \cite{conover_rank_1981,zimmerman_relative_1993}.
...
% Read http://www.bodowinter.com/tutorial/bw_LME_tutorial1.pdf and
% http://www.bodowinter.com/tutorial/bw_LME_tutorial2.pdf on linear
% mixed effects models and extend the following to be a more complete
% report of this output from the R:
%[1] "Anova on ranks"
%> rlatency <- rank(latdat$latency)
%> an2 <- lme(rlatency ~ condition_str, random = ~1|subj_id/condition_str, data=latdat)
%> print(anova(an2))
%              numDF denDF  F-value p-value
%(Intercept)       1   108 382.1123  <.0001
%condition_str     2   108 137.7248  <.0001
...
%> print(anova(an2_i))
%            numDF denDF   F-value p-value
%(Intercept)     1   108 14.809621   2e-04
%subj_id        54   108  5.337834  <.0001

% This was the report from the assumed-normal pairwise tests:
%A Bonferroni corrected post hoc test indicated that there were
%statistically significant differences between the No Distractor
%condition and each of the Synchronous and Asynchronous Distractor
%conditions (mean differences: -41.3, p\textless0.0005 [ND/SD]; -45.0,
%p\textless0.0005 [ND/AD]), but not between the Synchronous Distractor
%condition and the Asynchronous Distractor condition (mean
%difference=-3.6, p=1).

% NB: Is our distribution symmetrical? That's an assumption of this test.
...
0.000003 that the mean for the synchronous distractor latency should be
greater than that for the asynchronous distractor. The results for the
latency means are summarised in Table~\ref{table:latencies}.
...
maximum~\cite{hirsh_spatial_1989} and rod density is comparable with
...
The error rates are given in Table~\ref{table:error_rates}.
...
%\subsection*{Alternative latency extraction}
%The alternative latency extraction validated original movement
%analysis.

% FIXME: Convert this subsection into a much shorter paragraph
%\paragraph{Per-individual analysis.}
...
%individual to find behavioural clustering based on an analysis of
%whether the individual was statistically significantly faster to react
%in the `Asynchronous Distractor' condition compared with the
%`Synchronous Distractor' condition. Although we were able to identify
%small groups faster in one or other condition, there were no
%notable differences between the groups in their latency or error
%values other than those we had selected for.

% Taken out:
%Based on a comparison using
%bootstrap analyses of each individual's movement latencies, we found a
%group of 13 individuals who were significantly faster to react in the
%synchronous condition (`Faster Synchronous') and a group of 6
%individuals who were faster in the asynchronous condition (`Faster
%Asynchronous'). The remaining 36 individuals showed no significant
%difference in reaction time between these two distractor
%conditions. We found that group membership had no significant effect
%on the error rates. The `Faster Synchronous' group had a significantly
%faster movement response compared with the `No Difference' group in
%the synchronous distractor condition and a significantly slower
%movement response in the asynchronous distractor condition. The
%`Faster Asynchronous' group latencies were not significantly different
%in the asynchronous distractor condition, compared with the `No
%Difference' group, but were longer in the synchronous condition. These
%results suggest that some individuals appear better able to ignore
%asynchronous distractors, whereas others are able to make faster,
%accurate movement decisions in a two-alternative forced choice task;
%in neither case is the overall error rate significantly changed.
...
their theory, attention is initimitely bound to neural activity which
...
the idea that action and attention are linked. In their model of
selective
...
goals (for example, 'ignore the red distractor'). Additional
inhibitory processes are required to prevent intermediate reaches
between competing reach vectors which are too closely aligned for the
off-centre inhibition mechanism to determine a winner.
...
The threshold required for activation of M1 will be governed by
activity in related thalamo-cortical loops through the basal
...
could specify a movement to the left of 4~cm. In all conditions, the
...
does not imply the need for a movement, as it does in the synchronous
condition.
%
...
Although the line task can be accused of lacking ecological validity,
in that it does not represent tasks common to everyday life, the
simplicity of its design means that the influence of distractor
interference on action selection mechanisms can be probed. Future
implementations of the line task could vary the rate of distractor
event presentation, the number of distractor lines or distractor and
target colours and thicknesses.  Whilst not discussed here, the
dynamics of the trajectories recorded by the line task can be analysed
in detail. In conclusion, the line task offers an effective tool for
the detailed investigation of action selection for reaching movements.

\section*{Supporting Information}

\subsection*{Line task video}

\label{linetaskvid}
{\bf See:} \url{https://www.youtube.com/watch?v=9o3AKdDq9aE}

\subsection*{Line task data and software}
...
\bibitem{hoshi_distinctions_2007}
Hoshi E, Tanji J.
...
\bibitem{muller_microstimulation_2005}
