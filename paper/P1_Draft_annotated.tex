...
%DIF DEL P1_Draft_firstsub.tex   Thu Oct 27 15:09:32 2016
%DIF ADD P1_Draft.tex            Thu Feb 23 16:29:42 2017
...
\DIFaddbegin \DIFadd{Phillip J. Paine\textsuperscript{5},
Timothy J. Heaton\textsuperscript{5},
}\DIFaddend Sean Anderson\textsuperscript{3,2},
...
\\
\DIFaddbegin \bf{5} \DIFadd{School of Mathematics and Statistics, The University of Sheffield, Sheffield, UK
}\\\DIFaddend \bigskip
...
%DIF >  paragraph removed from here and replaced with:
\DIFaddend In this paper, we are concerned with the former, within-goal selection
choice. \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  Now say that there's a lot going on to do all this, and so it's
%DIF <  necessary to have experiments which allow you to investigate.
\DIFdel{Multiple neural systems are known to be involved in the selectionand
execution of
actions~\mbox{%DIFAUXCMD
\cite{cisek_cortical_2007,ueda_encoding_2003,hoshi_distinctions_2007}}%DIFAUXCMD
. Cisek
\& Kalaska~\mbox{%DIFAUXCMD
\cite{cisek_neural_2010} }%DIFAUXCMD
review neurophysiological
...
used as a
predictor for the outcome in a lever-based selection
task~\mbox{%DIFAUXCMD
\cite{wallis_rule_2003} }%DIFAUXCMD
and in eye
movements~\mbox{%DIFAUXCMD
\cite{basso_modulation_1998}}%DIFAUXCMD
. Similarly, brain regions
thought to be involved in motor planning remain active during the execution of
movement~\mbox{%DIFAUXCMD
\cite{cisek_neural_2010,hoshi_distinctions_2007}}%DIFAUXCMD
, and
microstimulation of brainstem regions can affect
attention~\mbox{%DIFAUXCMD
\cite{muller_microstimulation_2005}}%DIFAUXCMD
.  To help understand the interdependency,
and simultaneous operation of multiple brain regions
...
onset timing; see, for example~\mbox{%DIFAUXCMD
\cite{jax_hand_2007}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{We wanted to investigate motor control and action selection,
...
line. Here, we present the task, which we call the }\emph{\DIFadd{line task}}\DIFadd{,
along with a sample implementation of the task with a healthy cohort
of subjects}\DIFaddend .
...
\DIFdelbegin \DIFdel{even }\DIFdelend has a proposed
...
connected~\cite{gribble_hand-eye_2002}\DIFdelbegin \DIFdel{: }\DIFdelend \DIFaddbegin \DIFadd{. Although an observation of a
...
sequential~\mbox{%DIFAUXCMD
\cite{carey_eyehand_2000} }%DIFAUXCMD
and the reaching movement has
been shown to be specified prior to the onset of the
saccade~\mbox{%DIFAUXCMD
\cite{gribble_hand-eye_2002}}%DIFAUXCMD
. }\DIFaddend Reaching movements affect both
saccade trajectories~\cite{tipper_reaching_2001} and
latencies~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{fisk_organization_1985,neggers_ocular_2000} }%DIFAUXCMD
and they appear to }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{fisk_organization_1985,neggers_ocular_2000,snyder_eye-hand_2002}
}%DIFAUXCMD
and they may }\DIFaddend be planned in an eye-centered coordinate
system~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{batista_reach_1999}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{batista_reach_1999,tipper_reaching_2001}}%DIFAUXCMD
. Because of this
...
decision making processes}\DIFaddend .
...
\DIFdelbegin \DIFdel{It is clear that there is a need for a simplified yet detailed
framework to study task selection}\DIFdelend %DIF >  New section on step tracking experiments.
\DIFaddbegin \DIFadd{There does exist a class of experiments known as }\emph{\DIFadd{step-tracking
  tasks}} \DIFadd{which study reach movements in response to simple, quick
...
oscilloscope~\mbox{%DIFAUXCMD
\cite{trumbo_versatile_1963}}%DIFAUXCMD
...
elbow~\mbox{%DIFAUXCMD
\cite{trumbo_motor_1968}}%DIFAUXCMD
. Hallett and co-workers used a similar
apparatus to that used by Trumbo et al.~in an investigation of EMG
activity in biceps and triceps muscles~\mbox{%DIFAUXCMD
\cite{hallett_emg_1975}}%DIFAUXCMD
. Other
researchers performed step-tracking experiments for wrist movements,
tracking a target moving in one
dimension~\mbox{%DIFAUXCMD
\cite{waters_influence_1981,hoffman_step-tracking_1986} }%DIFAUXCMD
or
two~\mbox{%DIFAUXCMD
\cite{haruno_optimal_2005}}%DIFAUXCMD
. In each of these step-tracking
...
task forms a simplified framework by which task selection, }\DIFaddend as
expressed in limb movements and distinct from \DIFdelbegin \DIFdel{saccadic eye movements. Such a framework could be used
in
conjunction with measurements of saccadic eye movements to gain
further understanding of the interplay between the two systems}\DIFdelend \DIFaddbegin \DIFadd{(or indeed, together
with~\mbox{%DIFAUXCMD
\cite{trumbo_motor_1968}}%DIFAUXCMD
) saccadic eye movement, can be
...
natural reach-to-grasp movements}\DIFaddend .

Experiments to investigate \DIFdelbegin \DIFdel{both the action selection and motor
planning aspects of an arm movement }\DIFdelend \DIFaddbegin \DIFadd{the nature of arm movements }\DIFaddend are typically
laboratory based. \DIFdelbegin \DIFdel{They }\DIFdelend \DIFaddbegin \DIFadd{Like the step-tracking experiments described above,
they }\DIFaddend usually consist of a screen to display the stimuli and an input
...
Here, \DIFdelbegin \DIFdel{we address these }\DIFdelend \DIFaddbegin \DIFadd{addressing the }\DIFaddend issues of complexity\DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend lab-bound constraint and
\DIFaddbegin \DIFadd{target/effector end-point dissociation, we }\DIFaddend present the \emph{line
...
vertical lines which move in one dimension only (horizontally)\DIFdelbegin \DIFdel{and colour }\DIFdelend \DIFaddbegin \DIFadd{,
...
Colour }\DIFaddend is used to allow subjects to distinguish between target and
...
factors associated with higher level cognitive loading and \DIFaddbegin \DIFadd{explicit }\DIFaddend 3D
...
location. $T_t$ is \DIFdelbegin \DIFdel{given by}\DIFdelend \DIFaddbegin \DIFadd{drawn from a normal distribution with mean 1.6~s
and variance 0.636}\DIFaddend :
%
%DIF >  Rewriting:
%DIF >  T_t = A + B N(\mu, \sigma^2) = A + N (B\mu, B^2\sigma^2) = N(A+B\mu, B^2\sigma^2)
%DIF > 
%DIF >  so for A=0.8, B=0.005, \mu=160, \sigma^2=25440: T_t = N (\mu'=1.6, \sigma'^2=0.636)
\begin{equation}\DIFdelbegin %DIFDELCMD < \label{eq:tau_distractor_sync}
%DIFDELCMD <   %%%
\DIFdelend \DIFaddbegin \label{eq:tau_target}
  \DIFaddend T_t \DIFdelbegin \DIFdel{= 0.8 + 0.005 \times }\DIFdelend \DIFaddbegin \DIFadd{\sim }\DIFaddend N(\mu=\DIFdelbegin \DIFdel{160}\DIFdelend \DIFaddbegin \DIFadd{1.6}\DIFaddend ,\sigma^2=\DIFdelbegin \DIFdel{25440}\DIFdelend \DIFaddbegin \DIFadd{0.636}\DIFaddend )~\text{seconds}
\end{equation}
%
...
% variance 25440.
%
\DIFdelbegin \DIFdel{where $N(\mu=160,\sigma^2=25440)$ signifies a random 32 bit floating
...
of 1.6 seconds}\DIFdelend \DIFaddbegin \DIFadd{In the synchronous distractor condition, the time between distractor
location changes is identical to $T_t$}\DIFaddend . The new \DIFaddbegin \DIFadd{target }\DIFaddend location, $x$,
was computed by first generating a random location offset,
${\delta}x$\DIFdelbegin \DIFdel{:
}\DIFdelend \DIFaddbegin \DIFadd{, then adding this to the current location, subject to the
bounds of the edge of the screen:
%DIF >  See line 212 of line.cpp (of the Windows line task) for the code
%DIF >  which produces the following. Note that the final tests in the cpp
%DIF >  code (lines 215 & 216) are not necessary.
}\DIFaddend \begin{equation}\label{eq:rand_loc_offset_sync}
  {\delta}x \DIFdelbegin \DIFdel{= }\DIFdelend \DIFaddbegin \DIFadd{\sim }\DIFaddend \frac{w}{4}~U(-1,1)
\end{equation}
\begin{equation}\label{eq:new_loc_sync}
  x=
  \DIFdelbegin %DIFDELCMD < \begin{cases}
%DIFDELCMD <     x' + {\delta}x & \text{for }\frac{w}{10}<x<\frac{9w}{10}\\
%DIFDELCMD <     x' - {\delta}x & \text{otherwise}\\
%DIFDELCMD <   \end{cases}
%DIFDELCMD < %%%
...
distractor condition, the time between \DIFaddbegin \DIFadd{target location changes is the
same as in the ND and SD condition with $T_t \sim
N(\mu=1.6,\sigma^2=0.636)~\text{s}$. The time between }\DIFaddend distractor
...
%
%DIF >  Rewriting this:
%DIF >  T_d = A + B N(\mu, \sigma^2) = A + N (B\mu, B^2\sigma^2) = N(A+B\mu, B^2\sigma^2)
%DIF > 
%DIF >  so for A=0.4, B=0.005, \mu=80, \sigma^2=6320: T_d = N (\mu'=0.8, \sigma'^2=0.158)
%DIF > 
\begin{equation}\label{eq:tau_distractor_async}
  T_d \DIFdelbegin \DIFdel{= 0.4 + 0.005 \times }\DIFdelend \DIFaddbegin \DIFadd{\sim }\DIFaddend N(\mu=\DIFdelbegin \DIFdel{80}\DIFdelend \DIFaddbegin \DIFadd{0.8}\DIFaddend ,\sigma^2=\DIFdelbegin \DIFdel{6320}\DIFdelend \DIFaddbegin \DIFadd{0.158}\DIFaddend )~\text{seconds}
\end{equation}
Note the shorter periods; the asynchronous distractor changes location
more quickly than the target\DIFdelbegin \DIFdel{(the distractor mean jump time is 0.8
seconds)}\DIFdelend .
The locations of the asynchronous distractor are calculated according to:
\begin{equation}\label{eq:rand_loc_offset_async}
  {\delta}x \DIFdelbegin \DIFdel{= }\DIFdelend \DIFaddbegin \DIFadd{\sim }\DIFaddend \frac{w}{2}~U(-1,1)
\end{equation}
...
screen width\DIFaddbegin \DIFadd{, which allows the distractor to appear either side of the
next target location, even if that is at the most distal location
possible}\DIFaddend .  In the synchronous distractor condition, the distractor
...
\begin{figure}[htb!]
\centering
%DIF < \includegraphics[width=0.9\textwidth]{./figures/Fig1.tiff}
%DIF > \includegraphics[width=0.9\textwidth]{./figures/Fig1.jpg}
...
asynchronous and no-distractor). \DIFdelbegin \DIFdel{An additional random effects independent variable of `individual' was
used in Factorial Anova
analyses}\DIFdelend \DIFaddbegin \DIFadd{A linear mixed effects model was
fitted with distractor condition as the fixed effect and individual
included as the random effects component}\DIFaddend . An alpha value of 0.05 was
selected for significance testing.
%DIF <  Check wording of this statement.
...
%DIF > 
\DIFaddbegin \DIFadd{A software timer guaranteed that the stylus position was recorded at
...
precision of 1~ms.
}\DIFaddend % 60 Hz? Need to check on tablet.
The refresh rate of the tablet display screen was 60 s\DIFdelbegin \DIFdel{$^-1$}\DIFdelend \DIFaddbegin \DIFadd{$^{-1}$}\DIFaddend . The line
...
\begin{figure}[htb!]
\centering
%DIF < \includegraphics[width=0.5\textwidth]{./figures/Fig2.tiff}
%DIF > \includegraphics[width=1\textwidth]{./figures/Fig2.tiff}
\caption[Obtaining the latency, method 1] {\DIFaddbeginFL \textbf{\DIFaddFL{a)}} \DIFaddendFL An example
  event \DIFdelbeginFL \DIFdelFL{, showing
  the result of the movement onset analysis}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{--- number 18 from this data set}\DIFaddendFL . The horizontal position of
  ...
  position of the target\DIFdelbeginFL \DIFdelFL{line}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{. At event 18, the position of the target
  ...
  plot b). }\textbf{\DIFaddFL{b)}} \DIFaddFL{The expanded region from plot a) which shows
  the result of the movement onset analysis in more detail}\DIFaddendFL . Blue lines
  indicate the `stable stylus position' period which precedes the
  target event\DIFdelbeginFL \DIFdelFL{(number 18 from
  this data set)}\DIFdelendFL . Blue dot-dash lines indicate 1 standard deviation in
  ...
  with \DIFdelbeginFL \DIFdelFL{black arrows indicating the time for the motion to complete
  (long black dashes show }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{a purple arrow showing the algorithm's estimate of }\DIFaddendFL the length
  of the first, uninterrupted smooth motion \DIFdelbeginFL \DIFdelFL{; short dots show the time it took for the movement to arrive at the target)}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{and the orange arrow
  indicating the detected time for the motion to complete}\DIFaddendFL . Here, the
...
recorded as a movement error. \DIFaddbegin \DIFadd{Feedback on whether or not the movement
was in error was }\emph{\DIFadd{not}} \DIFadd{given to the subject during the task. }\DIFaddend Note
...
\begin{figure}[htb!]
\centering
%\includegraphics[width=0.5\textwidth]{./figures/Fig3.tiff}
...
\begin{figure}[htb!]
\centering
%\includegraphics[width=0.7\textwidth]{./figures/Fig4.tiff}
...
\DIFdelbegin \DIFdel{3~ms
%DIF <  We should probably understand this 3 ms difference better...
shorter (maybe because the method is better at detecting the exact
movement onset) and 6.6~ms shorter for the
distractor case}\DIFdelend \DIFaddbegin \DIFadd{systematically slightly shorter than the primary method due to
...
the alternative method alongside those for the primary method}\DIFaddend .
...
The \DIFdelbegin \DIFdel{task parameters and the latency and error data generated by the
Octave script were stored in a single file. An ipython notebook was
used to apply the various statistical analyses which are reported here . Some analyses }\DIFdelend \DIFaddbegin \DIFadd{statistical analyses reported here }\DIFaddend were carried out in
R~\cite{r_core_team_r:_2013} \DIFdelbegin \DIFdel{,
in which cases, the ipython notebook made calls to R, displaying the
results within the notebook}\DIFdelend \DIFaddbegin \DIFadd{and python}\DIFaddend .
...
\begin{figure}[htb!]
\centering
%\includegraphics[width=0.5\textwidth]{./figures/Fig5.tiff}
...
%DIF <  Olivia/BG/line/20141120153210.txt. Figure not shown for now.n
%DIF >  Olivia/BG/line/20141120153210.txt. Figure not shown for now.

\section*{Results}

\DIFaddbegin \DIFadd{Other than where indictated, the results presented here have been
derived from analysis of latencies and error rates produced by the
primary latency extraction method.
}

\DIFaddend \subsection*{Latency extraction}
...
each condition\DIFdelbegin \DIFdel{; these are henceforth referred to as the `sampled
data'}\DIFdelend .

\begin{figure}[htb!]
\centering
%\includegraphics[width=0.7\textwidth]{./figures/Fig6.tiff}
\caption[Latency measurement PDF] {Probability density \DIFdelbeginFL \DIFdelFL{functions }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{function
  approximations }\DIFaddendFL for 2013 latency measurements for each of three
  ...
  distractor. Dashed lines indicate the means. \DIFaddbeginFL \DIFaddFL{Although the
  ...
  distribution.}\DIFaddendFL }
\label{data_density}
\end{figure}

%DIF <  Add something here with the right amount of detail on mad.
\DIFdelbegin \DIFdel{To determine whether a one-way, repeated measures ANOVA could be
applied to the data, the data were analysed for normality. A
Shapiro-Wilks test was applied to 25 sub-samples of the
mean latency
measurements for each individual in each condition.
The null hypothesis was rejected for two out of three conditions (ND:
p=0.04, SD:
  p=0.001, AD: p=0.46). Therefore, a non-parametric test was
applied.}\DIFdelend \DIFaddbegin \DIFadd{Fig.~\ref{data_density_alt} shows the result of the same analysis of
...
\begin{figure}[htb!]
\centering
%DIF > \includegraphics[width=0.7\textwidth]{./figures/Fig7.tiff}
\caption[Latency measurement PDF] {\DIFaddFL{Probability density function
...
\begin{table}[ht]
\caption{\DIFaddFL{Summary of latency means with standard deviations, for the
...
\hline
\textbf{\DIFaddFL{Condition}} & \textbf{\DIFaddFL{Mean (ms)}} & \textbf{\DIFaddFL{Std. Dev.}} & \textbf{\DIFaddFL{Std. Err.}} \\ [\DIFaddFL{0.5ex}]
\hline
\DIFaddFL{~ }& \multicolumn{3}{c}{Primary latency extraction method} \\
\hline
\DIFaddFL{ND }& \DIFaddFL{295.1 }& \DIFaddFL{51 }& \DIFaddFL{1.1 }\\
\DIFaddFL{SD }& \DIFaddFL{336.0 }& \DIFaddFL{57 }& \DIFaddFL{1.3 }\\
\DIFaddFL{AD }& \DIFaddFL{345.0 }& \DIFaddFL{79 }& \DIFaddFL{1.8 }\\ [\DIFaddFL{1ex}]
\hline
\DIFaddFL{~ }& \multicolumn{3}{c}{Alternative latency extraction method} \\
\hline
\DIFaddFL{ND }& \DIFaddFL{292.6 }& \DIFaddFL{53 }& \DIFaddFL{1.3 }\\
\DIFaddFL{SD }& \DIFaddFL{329.9 }& \DIFaddFL{56 }& \DIFaddFL{1.5 }\\
\DIFaddFL{AD }& \DIFaddFL{341.6 }& \DIFaddFL{71 }& \DIFaddFL{2.0 }\\ [\DIFaddFL{1ex}]
\hline
\end{tabular}
\label{table:latmeans}
\end{table}

\DIFaddend %Normality of data:
% Shapiro-Wilks:
...
\centering
%DIF > \includegraphics[width=0.7\textwidth]{./figures/Fig8.tiff}
\caption[Linear mixed effects model residuals] {\DIFaddFL{Standardized residuals
...
\DIFaddend We constructed a linear mixed effects model on the \DIFdelbegin \DIFdel{ranked }\DIFdelend latencies to
determine whether there was a statistically significant difference in
latency to first movement predicted by the \DIFaddbegin \DIFadd{fixed factor of the }\DIFaddend three
conditions `No Distractor', `Synchronous Distractor' and `Asynchronous
Distractor'\DIFdelbegin \DIFdel{, with the random factor being the individual subject , in
which was nested the condition.
}%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  Read http://www.bodowinter.com/tutorial/bw_LME_tutorial1.pdf and
%DIF <  http://www.bodowinter.com/tutorial/bw_LME_tutorial2.pdf on linear
%DIF <  mixed effects models and extend the following to be a more complete
%DIF <  report of this output from the R:
%DIF < [1] "Anova on ranks"
%DIF < > rlatency <- rank(latdat$latency)
%DIF < > an2 <- lme(rlatency ~ condition_str, random = ~1|subj_id/condition_str, data=latdat)
%DIF < > print(anova(an2))
%DIF <               numDF denDF  F-value p-value
%DIF < (Intercept)       1   108 382.1123  <.0001
%DIF < condition_str     2   108 137.7248  <.0001
\DIFdelend \DIFaddbegin \DIFadd{. The individual subject was used as the random factor to
give an model of the form (using R-like notation):
}\begin{equation}\DIFadd{\label{eq:linear_mixed_effects_model}
\text{latency} \sim \text{condition} + (1|\text{subject}) + \epsilon
}\end{equation}
%DIF > 
%DIF > % > source ('Nonranked.r', print.eval=TRUE)
%DIF > % Linear mixed-effects model fit by REML
%DIF > %  Data: latdat
%DIF > %        AIC      BIC    logLik
%DIF > %   1513.105 1528.542 -751.5523
%DIF > %
%DIF > % Random effects:
%DIF > %  Formula: ~1 | subj_id
%DIF > %         (Intercept) Residual
%DIF > % StdDev:    22.41507 18.10252
%DIF > %
%DIF > % Fixed effects: latency ~ condition_str
%DIF > %                    Value Std.Error  DF   t-value p-value
%DIF > % (Intercept)     347.1905  3.885022 108  89.36643  0.0000
%DIF > % condition_strND -50.9244  3.452015 108 -14.75209  0.0000
%DIF > % condition_strSD  -9.5913  3.452015 108  -2.77847  0.0064
%DIF > %  Correlation:
%DIF > %                 (Intr) cnd_ND
%DIF > % condition_strND -0.444
%DIF > % condition_strSD -0.444  0.500
%DIF > %
%DIF > % Standardized Within-Group Residuals:
%DIF > %         Min          Q1         Med          Q3         Max
%DIF > % -1.84077745 -0.53695422 -0.01665622  0.51861454  3.00485476
%DIF > %
%DIF > % Number of Observations: 165
%DIF > % Number of Groups: 55
%DIF > %               numDF denDF  F-value p-value
%DIF > % (Intercept)       1   108 9615.924  <.0001
%DIF > % condition_str     2   108  122.904  <.0001
%DIF > %Data: latdat
%DIF > %Models:
%DIF > %nonranked2.null: latency ~ 1 + (1 | subj_id)
%DIF > %nonranked2.mdl: latency ~ condition_str + (1 | subj_id)
%DIF > %                Df    AIC    BIC  logLik deviance  Chisq Chi Df Pr(>Chisq)
%DIF > %nonranked2.null  3 1652.2 1661.5 -823.10   1646.2
%DIF > %nonranked2.mdl   5 1525.7 1541.2 -757.83   1515.7 130.53      2  < 2.2e-16 ***
%DIF > %---
%DIF > %Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1
%DIF > 
\DIFaddend The model showed that the different task conditions elicited
...
%DIF < > print(anova(an2_i))
%DIF <             numDF denDF   F-value p-value
%DIF < (Intercept)     1   108 14.809621   2e-04
%DIF < subj_id        54   108  5.337834  <.0001
}%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  This was the report from the assumed-normal pairwise tests:
%DIF < A Bonferroni corrected post hoc test indicated that there were
%DIF < statistically significant differences between the No Distractor
%DIF < condition and each of the Synchronous and Asynchronous Distractor
%DIF < conditions (mean differences: -41.3, p\textless0.0005 [ND/SD]; -45.0,
%DIF < p\textless0.0005 [ND/AD]), but not between the Synchronous Distractor
%DIF < condition and the Asynchronous Distractor condition (mean
%DIF < difference=-3.6, p=1).
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  NB: Is our distribution symmetrical? That's an assumption of this test.
\DIFdel{Wilcoxon signed rank tests were applied to determine differences
...
that the
small difference in latency between the SD and AD conditions
was also statistically significant (W=1042, p}\DIFdelend \DIFaddbegin \DIFadd{($\chi^2$(2)}\DIFaddend =\DIFdelbegin \DIFdel{0.022).}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{We applied Cliff's Delta~\mbox{%DIFAUXCMD
\cite{cliff_ordinal_2014} }%DIFAUXCMD
to the data for
...
the thresholds provided in \mbox{%DIFAUXCMD
\cite{romano_appropriate_2006}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{130.53, p\textless2.2$\times$10$^{-16}$). Plots of the
residuals from the model (Fig.~\ref{residuals}) do not suggest any
evidence for heteroscedasticity or non-normality}\DIFaddend .

\DIFdelbegin \DIFdel{Because the Wilcoxon signed rank test and Cliff's Delta indicated that
there is a significant difference between all pairs of
levels (ND, SD
and AD), }\DIFdelend \DIFaddbegin \DIFadd{To determine differences and confidence intervals between pairs of
conditions }\DIFaddend we applied a bootstrap analysis to the \DIFdelbegin \DIFdel{condition pairs. The
bootstrap method can give estimates }\DIFdelend \DIFaddbegin \DIFadd{data. We computed an
estimate }\DIFaddend of the standard error of the \DIFdelbegin \DIFdel{population mean (or any other population statistic) and confidence
intervals for the statistic. We resampled from the sampled data 2013
times in
each condition, each time computing the mean of the resampled
...
the no-distractor condition are shown in
black, synchronous distractor
...
0.000003 that the mean for }\DIFdelend \DIFaddbegin \DIFadd{difference between the means in
}\DIFaddend the \DIFdelbegin \DIFdel{synchronous distractor latency should
be greater than that for the asynchronous distractor . The results for the latency means are summarised in Table~\ref{table:latencies} and
presented, along with 95\% confidence intervals in Fig.~\ref{meanmad}}\DIFdelend \DIFaddbegin \DIFadd{three conditions. These results are summarised in
...
the no distractor condition, but small for the SD/AD pair}\DIFaddend .

\DIFdelbegin %DIFDELCMD < \begin{figure}[htb!]
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \begin{table}[ht]
\caption{\DIFaddFL{Summary of bootstrapped two-sample analyses of latency
  ...
  extraction method are shown here.}}  \DIFaddendFL \centering
%DIF < \includegraphics[width=0.7\textwidth]{./figures/Fig7.tiff}
\DIFdelbeginFL %DIFDELCMD < \caption[Bootstrapped PDFS] {%%%
\DIFdelFL{Probability density functions for the
  latency sample means computed from 2013 data resamples of the sampled data points for each condition. Data for the ND condition
  are shown in black, }\DIFdelendFL \DIFaddbeginFL \begin{tabular}{c c c c c}
\hline
\textbf{\DIFaddFL{Conditions}} & \textbf{\DIFaddFL{Difference (ms)}} & \textbf{\DIFaddFL{SE}} & \textbf{\DIFaddFL{Z-score}} & \textbf{\DIFaddFL{t-test p-value}} \\ [\DIFaddFL{0.5ex}]
\hline
\DIFaddFL{~ }& \multicolumn{4}{c}{Primary latency extraction method} \\
\hline
\DIFaddFL{ND/}\DIFaddendFL SD \DIFdelbeginFL \DIFdelFL{in blue and AD in red. Dashed lines show the sample means; dot-dash lines show the 99.83}\DIFdelendFL \DIFaddbeginFL & \DIFaddFL{40.9 }& \DIFaddFL{1.67 }& \DIFaddFL{24.5 }& \DIFaddFL{\textless2.4$\times$10$^{-7}$ }\\
\DIFaddFL{ND/AD }& \DIFaddFL{49.8 }& \DIFaddFL{2.07 }& \DIFaddFL{24.1 }& \DIFaddFL{\textless2.4$\times$10$^{-7}$ }\\
\DIFaddFL{SD/AD }& \DIFaddFL{8.92 }& \DIFaddFL{2.24 }& \DIFaddFL{3.98 }& \DIFaddFL{0.000015 }\\ [\DIFaddFL{1ex}]
\hline
\DIFaddFL{~ }& \multicolumn{4}{c}{Alternative latency extraction method} \\
\hline
\DIFaddFL{ND/SD }& \DIFaddFL{37.3 }& \DIFaddFL{2.00 }& \DIFaddFL{24.5 }& \DIFaddFL{\textless1$\times$10$^{-3}$ }\\
\DIFaddFL{ND/AD }& \DIFaddFL{49.0 }& \DIFaddFL{2.34 }& \DIFaddFL{20.9 }& \DIFaddFL{\textless1$\times$10$^{-3}$ }\\
\DIFaddFL{SD/AD }& \DIFaddFL{11.6 }& \DIFaddFL{2.45 }& \DIFaddFL{4.73 }& \DIFaddFL{\textless1$\times$10$^{-3}$ }\\ [\DIFaddFL{1ex}]
\hline
\end{tabular}
\label{table:twosample}
\end{table}

\DIFaddFL{To obtain a better estimate of the probability that the distributions
...
(4$\times$10$^{6}$ resamples). The mean latencies, with 95}\DIFaddendFL \%
confidence intervals \DIFdelbeginFL \DIFdelFL{for }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{computed from a bootstrap analysis of }\DIFaddendFL the sample
means\DIFdelbeginFL \DIFdelFL{. This confidence interval was
  chosen to illustrate the value for which the confidence intervals of
  the AD and SD distributions `touch'.
}%DIFDELCMD < }
%DIFDELCMD < %DIFDELCMD < \label{bootstrapmean}%%%
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{, are shown in Fig.~\ref{meanmad}.
}\DIFaddend 
...
\centering
%DIF < \includegraphics[width=0.7\textwidth]{./figures/Fig8.tiff}
%DIF > \includegraphics[width=0.7\textwidth]{./figures/Fig9.tiff}
\caption[Mean latencies] {Mean latencies to first movement in the
  three experimental conditions `No Distractor' (ND), `Synchronous
  Distractor' (SD) and `Asynchronous Distractor' (AD) \DIFaddbeginFL \DIFaddFL{as determined
  using the primary latency extraction method}\DIFaddendFL . In this plot, the
  error bars show 95\% confidence intervals, \DIFdelbeginFL \DIFdelFL{again determined by the }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{determined by a }\DIFaddendFL bootstrap
  analysis of the sample means\DIFdelbeginFL \DIFdelFL{as illustrated in
  Fig.~\ref{bootstrapmean}}\DIFdelendFL .}
\label{meanmad}
\end{figure}
%% This plot generated in Bootstrap_all.r

%DIF > %
%DIF > % ND error vs ND no error: p=0.005
%DIF > % AD vs AD: p=0.0001
%DIF > % SD vs SD: p<0.00002
%DIF > %
...
for correct movements (p\textless0\DIFdelbegin \DIFdel{.001 SD,}\DIFdelend \DIFaddbegin \DIFadd{.00002 SD; p=0.0001 }\DIFaddend AD; p\DIFdelbegin \DIFdel{\textless0.01 }\DIFdelend \DIFaddbegin \DIFadd{=0.005 }\DIFaddend ND) by
about \DIFdelbegin \DIFdel{70}\DIFdelend \DIFaddbegin \DIFadd{100}\DIFaddend ~ms. Refer to Table~\ref{table:latencies} for the results.

%DIF > % Contains error and non-error. Generated by Bootstrap_two_errornoerror.r
\begin{table}[ht]
\caption{Mean latencies \DIFdelbeginFL \DIFdelFL{with corresponding standard deviation (SD),
  median absolute deviation }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{for movements directed towards the target
  }\DIFaddendFL (\DIFdelbeginFL \DIFdelFL{MAD}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{non-error}\DIFaddendFL ) and \DIFdelbeginFL \DIFdelFL{95\% confidence interval
  statistics (CI)}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{for movements which were in error and directed away
  ...
  the bootstrap method described in the text}\DIFaddendFL .}  \centering
\DIFdelbeginFL %DIFDELCMD < \begin{tabular}{c c c c c}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \begin{tabular}{c c c c c c}
\DIFaddendFL \hline
\textbf{Condition} & \DIFdelbeginFL \textbf{\DIFdelFL{Mean (ms)}} %DIFAUXCMD
\DIFdelendFL \DIFaddbeginFL \multicolumn{2}{c}{\textbf{Mean latency (ms)}} \DIFaddendFL & \textbf{\DIFdelbeginFL \DIFdelFL{SD}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Difference}\DIFaddendFL } & \textbf{\DIFdelbeginFL \DIFdelFL{MAD}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{std. err.}\DIFaddendFL } & \textbf{\DIFdelbeginFL \DIFdelFL{95\% CI}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{p value}\DIFaddendFL } \\ [0.5ex]
\DIFdelbeginFL %DIFDELCMD < \hline
%DIFDELCMD < \multicolumn{5}{c}{\emph{Non-error movements}} \\
%DIFDELCMD < \hline
%DIFDELCMD < %%%
\DIFdelFL{No Distractor           }%DIFDELCMD < & %%%
\DIFdelFL{295.1 }%DIFDELCMD < & %%%
\DIFdelFL{51 }%DIFDELCMD < & %%%
\DIFdelFL{44 }%DIFDELCMD < & %%%
\DIFdelFL{1.8 }%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelFL{Synchronous Distractor  }%DIFDELCMD < & %%%
\DIFdelFL{336.0 }%DIFDELCMD < & %%%
\DIFdelFL{57 }%DIFDELCMD < & %%%
\DIFdelFL{50 }%DIFDELCMD < & %%%
\DIFdelFL{2.1 }%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelFL{Asynchronous Distractor }%DIFDELCMD < & %%%
\DIFdelFL{345.0 }\DIFdelendFL \DIFaddbeginFL \multicolumn{1}{c}{~} \DIFaddendFL & \DIFdelbeginFL \DIFdelFL{79 }\DIFdelendFL \DIFaddbeginFL \textbf{\DIFaddFL{Non-error}} \DIFaddendFL & \DIFdelbeginFL \DIFdelFL{65 }\DIFdelendFL \DIFaddbeginFL \textbf{\DIFaddFL{Error}} \DIFaddendFL & \DIFdelbeginFL \DIFdelFL{2.9 }%DIFDELCMD < \\ [%%%
\DIFdelFL{1ex}%DIFDELCMD < ]
%DIFDELCMD < \hline
%DIFDELCMD < \multicolumn{5}{c}{\emph{Error movements}} %%%
...
\DIFaddbegin \begin{figure}[htb!]
\centering
%DIF > \includegraphics[width=0.7\textwidth]{./figures/Fig10.tiff}
\caption[Mean latencies vs. distance] {\DIFaddFL{`Moving bin' mean latencies to
...
\DIFaddend To examine whether the latency has a dependence on the distance
...
\DIFaddbegin \DIFadd{We repeated this analysis to determine if there is any correlation
...
\centering
%DIF < \includegraphics[width=0.7\textwidth]{./figures/Fig9.tiff}
%DIF > \includegraphics[width=0.7\textwidth]{./figures/Fig11.tiff}
\caption[Mean latencies vs. distance] {`Moving bin' mean latencies to
  first movement versus distance \DIFdelbeginFL \DIFdelFL{from stylus to target.A bin of width
  5~mm is moved through the data. At each distance, }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{computed as in Fig.~\ref{latvsdist}
  ...
  (red), which is }\DIFaddendFL the \DIFdelbeginFL \DIFdelFL{mean is
  computed (thicker lines)and the standard error of the mean is computed using the bootstrap method.The thinner lines show the corresponding 95\% confidence intervals about the mean. The
  }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{same data as the red plot in
  ...
  }\DIFaddendFL approximate location of the foveal region is shown by the dashed
  line\DIFdelbeginFL \DIFdelFL{, assuming that the eye to screen distance is 400~mm. A marked
  increase in latency is seen for all conditions within the foveal
  region. In the non-foveal region, distance is predictive only for
  the asynchronous distractor}\DIFdelendFL .}
\DIFdelbeginFL %DIFDELCMD < %DIFDELCMD < \label{latvsdist}%%%
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \label{latvslastdist}
...
\centering
%DIF < \includegraphics[width=0.7\textwidth]{./figures/Fig10.tiff}
%DIF > \includegraphics[width=0.7\textwidth]{./figures/Fig12.tiff}
...
%DIF < \subsection*{Alternative latency extraction}
%DIF < The alternative latency extraction validated original movement
%DIF < analysis.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  FIXME: Convert this subsection into a much shorter paragraph
%DIF < \paragraph{Per-individual analysis.}
%DIF < Because the linear mixed effects model showed that both the condition
%DIF < and the individual contributed a similar amount of variation to the
%DIF < latency, we conducted an analysis of latency values within each
%DIF < individual to find behavioural clustering based on an analysis of
%DIF < whether the individual was statistically significantly faster to react
%DIF < in the `Asynchronous Distractor' condition compared with the
%DIF < `Synchronous Distractor' condition. Although we were able to identify
%DIF < small groups faster in one or other condition, there were no
%DIF < notable differences between the groups in their latency or error
%DIF < values other than those we had selected for.
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  Taken out:
%DIF < Based on a comparison using
%DIF < bootstrap analyses of each individual's movement latencies, we found a
%DIF < group of 13 individuals who were significantly faster to react in the
%DIF < synchronous condition (`Faster Synchronous') and a group of 6
%DIF < individuals who were faster in the asynchronous condition (`Faster
%DIF < Asynchronous'). The remaining 36 individuals showed no significant
%DIF < difference in reaction time between these two distractor
%DIF < conditions. We found that group membership had no significant effect
%DIF < on the error rates. The `Faster Synchronous' group had a significantly
%DIF < faster movement response compared with the `No Difference' group in
%DIF < the synchronous distractor condition and a significantly slower
%DIF < movement response in the asynchronous distractor condition. The
%DIF < `Faster Asynchronous' group latencies were not significantly different
%DIF < in the asynchronous distractor condition, compared with the `No
%DIF < Difference' group, but were longer in the synchronous condition. These
%DIF < results suggest that some individuals appear better able to ignore
%DIF < asynchronous distractors, whereas others are able to make faster,
%DIF < accurate movement decisions in a two-alternative forced choice task;
%DIF < in neither case is the overall error rate significantly changed.
%DIFDELCMD < 
...
behaviour within an action-selection framework \DIFaddbegin \DIFadd{in which we consider
the action-space to be one-dimensional}\DIFaddend .
...
\DIFdelbegin \DIFdel{The trajectory
...
presentation in the line task might differ \DIFaddbegin \DIFadd{and thus cannot help to
explain the small difference in latencies which we detected between
the SD and AD conditions in this work}\DIFaddend . The `response activation
...
distractor condition \DIFaddbegin \DIFadd{(by 50~ms; see Table~\ref{table:twosample})}\DIFaddend . Note
...
Cisek~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{cisek_cortical_2007} }%DIFAUXCMD
and based on
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{cisek_cortical_2007,cisek_integrated_2006} }%DIFAUXCMD
and supported
by }\DIFaddend neural activity reported in \cite{cisek_neural_2005} shows activity
...
a further increase in activity in PMd \DIFdelbegin \DIFdel{, which excites activity in
}\DIFdelend \DIFaddbegin \DIFadd{and ungates the connection from
PMd to }\DIFaddend primary motor cortex (M1), providing the motor command. The
...
\DIFdelbegin \DIFdel{In the asynchronous distractor condition, }\DIFdelend \DIFaddbegin \DIFadd{It is interesting to consider what }\DIFaddend Cisek's model would predict \DIFdelbegin \DIFdel{latencies comparable with the no-distractor condition for most target events because distractor events are not aligned in time
...
no-distractor condition and even significantly longer than in
the
...
threshold }\emph{\DIFdel{or}} %DIFAUXCMD
\DIFdel{a reduction in the
rate of accumulation of evidence}\DIFdelend \DIFaddbegin \DIFadd{for in
...
\mbox{%DIFAUXCMD
\cite{cisek_integrated_2006}}%DIFAUXCMD
). We would expect to see activity in the
...
with the asynchronous condition; a reverse of the observed trend}\DIFaddend .
...
patients. The apparent limitation of using \DIFdelbegin \DIFdel{using }\DIFdelend one dimensional
...
\DIFdelbegin \DIFdel{colours and }\DIFdelend thicknesses could be varied. \DIFaddbegin \DIFadd{A future experiment should be run to
...
conditions. }\DIFaddend Gaps between target and distractor presentation could be
...
%DIF > %% INCLUDE .bbl file here for submission, but for writing, do it the usual way:
%DIF > % The bibliography NoTremor.bib is the one exported from Zotero.
%DIF > %\bibliography{NoTremor}
...
\DIFdelbegin %DIFDELCMD < \bibitem{ueda_encoding_2003}
%DIFDELCMD < %%%
...
\DIFdel{Available from:
  }%DIFDELCMD < \url{http://onlinelibrary.wiley.com/doi/10.1046/j.1460-9568.2003.02814.x/abstract}%%%
\DIFdel{.
}%DIFDELCMD < 

%DIFDELCMD < \bibitem{hoshi_distinctions_2007}
%DIFDELCMD < %%%
...
\DIFdel{Available from:
  }%DIFDELCMD < \url{http://www.sciencedirect.com/science/article/pii/S095943880700027X}%%%
\DIFdel{.
}%DIFDELCMD < 

%DIFDELCMD < \bibitem{cisek_neural_2010}
%DIFDELCMD < %%%
...
%DIFDELCMD < \bibitem{wallis_rule_2003}
%DIFDELCMD < %%%
\DIFdel{Wallis JD, Miller EK.
}%DIFDELCMD < \newblock %%%
\DIFdel{From }%DIFDELCMD < {%%%
\DIFdel{Rule}%DIFDELCMD < } %%%
\DIFdel{to }%DIFDELCMD < {%%%
\DIFdel{Response}%DIFDELCMD < }%%%
\DIFdel{: }%DIFDELCMD < {%%%
\DIFdel{Neuronal}%DIFDELCMD < } {%%%
\DIFdel{Processes}%DIFDELCMD < } %%%
\DIFdel{in the }%DIFDELCMD < {%%%
\DIFdel{Premotor}%DIFDELCMD < }
%DIFDELCMD <   %%%
\DIFdel{and }%DIFDELCMD < {%%%
\DIFdel{Prefrontal}%DIFDELCMD < } {%%%
\DIFdel{Cortex}%DIFDELCMD < }%%%
\DIFdel{.
}%DIFDELCMD < \newblock %%%
\DIFdel{Journal of Neurophysiology. 2003 Sep;90(3):1790--1806.
}%DIFDELCMD < \newblock %%%
\DIFdel{Available from:
  }%DIFDELCMD < \url{http://jn.physiology.org/content/90/3/1790.abstract}%%%
\DIFdel{.
}%DIFDELCMD < 

%DIFDELCMD < \bibitem{basso_modulation_1998}
%DIFDELCMD < %%%
...
\DIFdel{Available from:
  }%DIFDELCMD < \url{http://www.jneurosci.org/content/18/18/7519.short}%%%
\DIFdel{.
}%DIFDELCMD < 

%DIFDELCMD < \bibitem{muller_microstimulation_2005}
%DIFDELCMD < %%%
...
\DIFdel{Available from: }%DIFDELCMD < \url{http://www.pnas.org/content/102/3/524.short}%%%
\DIFdel{.
}%DIFDELCMD < 

%DIFDELCMD < \bibitem{jax_hand_2007}
%DIFDELCMD < %%%
...
%DIFDELCMD < %%%
\DIFdelend \bibitem{deubel_saccade_1996}
...
\DIFaddbegin \bibitem{carey_eyehand_2000}
\DIFadd{Carey DP.
}\newblock \DIFadd{Eye–hand coordination: }{\DIFadd{Eye}} \DIFadd{to hand or hand to eye?
}\newblock \DIFadd{Current Biology. 2000;10(11):R416--R419.
}\newblock \DIFadd{Available from:
  }\url{http://www.sciencedirect.com/science/article/pii/S096098220000508X}\DIFadd{.
}

\DIFaddend \bibitem{tipper_reaching_2001}
...
\DIFaddbegin \bibitem{snyder_eye-hand_2002}
\DIFadd{Snyder LH, Calton JL, Dickinson AR, Lawrence BM.
}\newblock \DIFadd{Eye-}{\DIFadd{Hand}} {\DIFadd{Coordination}}\DIFadd{: }{\DIFadd{Saccades}} {\DIFadd{Are}} {\DIFadd{Faster}} {\DIFadd{When}}
  {\DIFadd{Accompanied}} \DIFadd{by a }{\DIFadd{Coordinated}} {\DIFadd{Arm}} {\DIFadd{Movement}}\DIFadd{.
}\newblock \DIFadd{Journal of Neurophysiology. 2002 May;87(5):2279.
}\newblock \DIFadd{Available from:
  }\url{http://jn.physiology.org/content/87/5/2279.abstract}\DIFadd{.
}

\DIFaddend \bibitem{batista_reach_1999}
...
\DIFaddbegin \bibitem{trumbo_versatile_1963}
\DIFadd{Trumbo D, Eslinger R, Noble M, Cross K.
}\newblock \DIFadd{A }{\DIFadd{Versatile}} {\DIFadd{Electronic}} {\DIFadd{Tracking}} {\DIFadd{Apparatus}} \DIFadd{(}{\DIFadd{VETA}}\DIFadd{).
}\newblock \DIFadd{Perceptual and Motor Skills. 1963 Jun;16(3):649--656.
}\newblock \DIFadd{Available from:
  }\url{http://journals.sagepub.com/doi/abs/10.2466/pms.1963.16.3.649}\DIFadd{.
}

\bibitem{trumbo_motor_1968}
\DIFadd{Trumbo D, Noble M, Fowler F, Porterfield J.
}\newblock \DIFadd{Motor performance on temporal tasks as a function of sequence length
  and coherence.
}\newblock \DIFadd{Journal of experimental psychology. 1968;77(3p1):397.
}

\bibitem{hallett_emg_1975}
\DIFadd{Hallett M, Shahani BT, Young RR.
}\newblock {\DIFadd{EMG}} \DIFadd{analysis of stereotyped voluntary movements in man.
}\newblock \DIFadd{Journal of Neurology, Neurosurgery \& Psychiatry. 1975
  Dec;38(12):1154--1162.
}\newblock \DIFadd{Available from:
  }\url{http://jnnp.bmj.com/content/38/12/1154.abstract}\DIFadd{.
}

\bibitem{waters_influence_1981}
\DIFadd{Waters P, Strick PL.
}\newblock \DIFadd{Influence of ‘strategy’on muscle activity during ballistic
  movements.
}\newblock \DIFadd{Brain research. 1981;207(1):189--194.
}

\bibitem{hoffman_step-tracking_1986}
\DIFadd{Hoffman D, Strick P.
}\newblock \DIFadd{Step-tracking movements of the wrist in humans. }{\DIFadd{I}}\DIFadd{. }{\DIFadd{Kinematic}}
  \DIFadd{analysis.
}\newblock \DIFadd{The Journal of Neuroscience. 1986 Nov;6(11):3309.
}\newblock \DIFadd{Available from:
  }\url{http://www.jneurosci.org/content/6/11/3309.abstract}\DIFadd{.
}

\bibitem{haruno_optimal_2005}
\DIFadd{Haruno M, Wolpert DM.
}\newblock \DIFadd{Optimal }{\DIFadd{Control}} \DIFadd{of }{\DIFadd{Redundant}} {\DIFadd{Muscles}} \DIFadd{in }{\DIFadd{Step}}\DIFadd{-}{\DIFadd{Tracking}}
  {\DIFadd{Wrist}} {\DIFadd{Movements}}\DIFadd{.
}\newblock \DIFadd{Journal of Neurophysiology. 2005 Nov;94(6):4244.
}\newblock \DIFadd{Available from:
  }\url{http://jn.physiology.org/content/94/6/4244.abstract}\DIFadd{.
}

\DIFaddend \bibitem{tipper_selective_1992}
...
\DIFaddbegin \bibitem{jax_hand_2007}
\DIFadd{Jax SA, Rosenbaum DA.
}\newblock \DIFadd{Hand path priming in manual obstacle avoidance: evidence that the
  dorsal stream does not only control visually guided actions in real time.
}\newblock \DIFadd{Journal of Experimental Psychology: Human Perception and Performance.
  2007;33(2):425.
}

\DIFaddend \bibitem{prablanc_optimal_1979}
...
\newblock In: The {ASQC} {Basic} {References} in {Quality} {Control}:
  ...
  vol.~16. ASQC Quality Press; 1993. \DIFaddbegin \DIFadd{p}\DIFaddend . \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < \bibitem{cliff_ordinal_2014}
%DIFDELCMD < %%%
\DIFdel{Cliff N.
}%DIFDELCMD < \newblock %%%
\DIFdel{Ordinal methods for behavioral data analysis.
}%DIFDELCMD < \newblock %%%
\DIFdel{Psychology Press; 2014.
}%DIFDELCMD < 

%DIFDELCMD < \bibitem{romano_appropriate_2006}
%DIFDELCMD < %%%
\DIFdel{Romano J, Kromrey JD, Coraggio J, Skowronek J.
}%DIFDELCMD < \newblock %%%
\DIFdel{Appropriate statistics for ordinal level data: }%DIFDELCMD < {%%%
\DIFdel{Should}%DIFDELCMD < } %%%
\DIFdel{we really be
  using t-test and }%DIFDELCMD < {%%%
\DIFdel{Cohen}%DIFDELCMD < }%%%
\DIFdel{’s d for evaluating group differences on the }%DIFDELCMD < {%%%
\DIFdel{NSSE}%DIFDELCMD < }
%DIFDELCMD <   %%%
\DIFdel{and other surveys; 2006. p. 1--33}\DIFdelend \DIFaddbegin \DIFadd{1--87}\DIFaddend .
...
Jonas JB, Schneider U, Naumann \DIFdelbegin \DIFdel{GO}\DIFdelend \DIFaddbegin \DIFadd{GOH}\DIFaddend .
\newblock Count and density of human retinal photoreceptors.
\newblock Graefe's \DIFdelbegin \DIFdel{archive for clinical and experimental ophthalmology}\DIFdelend \DIFaddbegin \DIFadd{Archive for Clinical and Experimental Ophthalmology}\DIFaddend .
  1992\DIFdelbegin \DIFdel{Jan}\DIFdelend ;230(6):505--510.
\newblock Available from: \DIFdelbegin %DIFDELCMD < \url{http://www.ncbi.nlm.nih.gov/pubmed/1427131}%%%
\DIFdelend \DIFaddbegin \url{http://dx.doi.org/10.1007/BF00181769}\DIFaddend .
...
\DIFaddbegin \bibitem{cisek_integrated_2006}
\DIFadd{Cisek P.
}\newblock \DIFadd{Integrated }{\DIFadd{Neural}} {\DIFadd{Processes}} \DIFadd{for }{\DIFadd{Defining}} {\DIFadd{Potential}} {\DIFadd{Actions}}
  \DIFadd{and }{\DIFadd{Deciding}} \DIFadd{between }{\DIFadd{Them}}\DIFadd{: }{\DIFadd{A}} {\DIFadd{Computational}} {\DIFadd{Model}}\DIFadd{.
}\newblock \DIFadd{Journal of Neuroscience. 2006 Sep;26(38):9761--9770.
}\newblock \DIFadd{Available from:
  }\url{http://www.jneurosci.org/cgi/doi/10.1523/JNEUROSCI.5605-05.2006}\DIFadd{.
}

\DIFaddend \bibitem{parent_functional_1995}
Parent A, Hazrati LN.
